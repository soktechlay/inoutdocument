                ...
                if (!empty($_POST["username"]) && !empty($_POST["password"])) {
                    ...
                    if ($account) {
                        ...
                        if ($account['Status'] == 'locked') {
                            ...
                            // Check if the account is supperadmin
                            if ($accountType === 'supperadmin') {
                                // Set session variables for supperadmin
                                $_SESSION['userid'] = $account['id'];
                                $_SESSION['role'] = 'supperadmin';
                                $_SESSION['permission'] = 'all'; // Or a specific permission for supperadmin
                                $_SESSION['username'] = $this->getFullName($account);

                                // Redirect to the supperadmin dashboard
                                header('Location: ../views/supperadmin/dashboard.php');
                                exit;
                            }

                            // Handle regular users/admins
                            if (isset($account['authenticator_enabled']) && $account['authenticator_enabled'] == 1) {
                                ...
                                // Set session variables for regular users/admins
                                $_SESSION['userid'] = $account['id'];
                                $_SESSION['role'] = $account['RoleName'];
                                $_SESSION['permission'] = $account['PermissionId'] ?? null;
                                $_SESSION['username'] = $this->getFullName($account);
