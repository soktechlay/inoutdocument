...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['login_type']) && $_POST['login_type'] == 'edit_report') {
  ...
  if (count($updatedHeadlines) !== count($updatedQuillData)) {
    ...
    header('Location: edit2.php?id=' . $request_id . '&error=1');
    exit();
  }

  try {
    // Prepare statement to update Quill data
    $stmt = $dbh->prepare("UPDATE tblreport_step2 SET data = :data WHERE request_id = :id");
    ...
    // Update headlines
    $stmt = $dbh->prepare("UPDATE tblreport_step2 SET headline = :headline WHERE request_id = :id");
