...
$pageTitle = "ឯកសារចូលការិយាល័យសវនកម្មទី៣";
$sidebar = "inofaudit3";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit"])) {
    ...
    if (move_uploaded_file($file_tmp, $destination)) {
        ...
        try {
            ...
            header("Location: inofaudit3.php?msg=" . urlencode($msg) . "&status=success");
            exit();
        } catch (PDOException $e) {
            $error = "Error: " . $e->getMessage();
            // Redirect with error message
            header("Location: inofaudit3.php?msg=" . urlencode($error) . "&status=error");
            exit();
        }
    } else {
...
if (isset($_POST['edit'])) {
    ...
    if ($query->rowCount() > 0) {
        header("Location: inofaudit3.php?ID=" . urlencode($id) . "&msg=Successfully+Edited&status=success");
        exit();
    } else {
        header("Location: inofaudit3.php?ID=" . urlencode($id) . "&msg=Error+Edited&status=failed");
        exit();
    }
}
        ...
        <div class="container-xl flex-grow-1">
            <div class="d-flex align-items-center justify-content-between">
                <div class="card-header">
                    <h4 class="py-3 mb-1 text-primary"><span class="text-muted fw-light ">ការិយាល័យសវនកម្មទី៣/</span>ឯកសារចូល</h4>
                </div>
