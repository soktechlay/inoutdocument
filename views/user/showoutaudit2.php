...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (isset($_POST['delete'])) {
    ...
    if ($query->rowCount() > 0) {
      header("Location: outaudit2.php?msg=Successfully+Deleted&status=success");
      exit();
    } else {
      header("Location: outaudit2.php?msg=Error:+Something+went+wrong&status=failed");
      exit();
    }
  ...
  WHERE ID = ?");
    ...
    if ($query->rowCount() > 0) {
      header("Location: showoutaudit2.php?ID=" . urlencode($id) . "&msg=Successfully+Edited&status=success");
      exit();
    } else {
      header("Location: showoutaudit2.php?ID=" . urlencode($id) . "&msg=Error+Edited&status=failed");
      exit();
    }
  }
}
