...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit"])) {
  ...
  if (move_uploaded_file($file_tmp, $destination)) {
    ...
    try {
      ...
      header("Location: intraning.php?msg=" . urlencode($error) . "&status=error");
      exit();
    }
  } else {
...
if (isset($_GET['delete'])) {
  ...
  if ($query->rowCount()) {
    ...
    // Redirect with success message
    header("Location: intraning.php?msg=" . urlencode($msg) . "&status=success");
    exit();
  } else {
...
if (isset($_POST['edit'])) {
  ...
  if ($query->rowCount() > 0) {
    header("Location: intraning.php?ID=" . urlencode($id) . "&msg=Successfully+Edited&status=success");
    exit();
  } else {
    header("Location: intraning.php?ID=" . urlencode($id) . "&msg=Error+Edited&status=failed");
    exit();
  }
}
