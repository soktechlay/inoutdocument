...
if (isset($_POST['delete'])) {
    ...
    if ($query->rowCount() > 0) {
        header("Location: inaudit2.php?msg=Successfully+Deleted&status=success");
        exit();
    } else {
        header("Location: inaudit2.php?msg=Error:+Something+went+wrong&status=failed");
        exit();
    }
}
...
if (isset($_POST['edit'])) {
    ...
    if ($query->rowCount() > 0) {
        header("Location: showinaudit2.php?ID=" . urlencode($id) . "&msg=Successfully+Edited&status=success");
        exit();
      } else {
        header("Location: showinaudit2.php?ID=" . urlencode($id) . "&msg=Error+Edited&status=failed");
        exit();
      }
}
                    ...
                    <form id="formAccountSettings" method="post">
                        ...
                        <div class="mt-2">
                            ...
                            <div class="col-md-12 text-end">
                                ...
                                <a href="inaudit2.php" class="btn btn-secondary">ត្រឡប់ក្រោយ</a>
                            </div>
                        </div>
                    </form>
                </div>
