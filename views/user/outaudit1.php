...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  WHERE ID = ?");
    ...
    if ($query->rowCount() > 0) {
      header("Location: outaudit1.php?ID=" . urlencode($id) . "&msg=Successfully+Edited&status=success");
      exit();
    } else {
      header("Location: outaudit1.php?ID=" . urlencode($id) . "&msg=Error+Edited&status=failed");
      exit();
    }
  }
}
