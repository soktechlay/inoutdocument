...
// Page variables
$pageTitle = "ឯកសារចេញការិយាល័យសវនកម្មទី៤";
$sidebar = "outofaudit4";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit"])) {
    ...
    if (move_uploaded_file($file_tmp, $destination)) {
        ...
        try {
            ...
            header("Location: outofaudit4.php?msg=" . urlencode($msg) . "&status=success");
            exit();
        } catch (PDOException $e) {
            $error = "Error: " . $e->getMessage();
            header("Location: outofaudit4.php?msg=" . urlencode($error) . "&status=error");
            exit();
        }
    } else {
...
if (isset($_POST['edit'])) {
  ...
  WHERE ID = ?");
    ...
    if ($query->rowCount() > 0) {
        header("Location: outofaudit4.php?ID=" . urlencode($id) . "&msg=Successfully+Edited&status=success");
        exit();
    } else {
        header("Location: outofaudit4.php?ID=" . urlencode($id) . "&msg=Error+Edited&status=failed");
        exit();
    }
}
        ...
        <div class="container-xl flex-grow-1">
            <div class="d-flex align-items-center justify-content-between">
                <div class="card-header">
                    <h4 class="py-3 mb-1 text-primary"><span class="text-muted fw-light ">ការិយាល័យសវនកម្មទី៤/</span>ឯកសារចេញ</h4>
                </div>
